...
class ComprehensiveHyperparameterTuner:
    ...
    def create_breakthrough_search_space(self) -> List[HyperparameterConfig]:
        ...
        # 🔥 BREAKTHROUGH-FOCUSED PARAMETER GRID
        param_grid = {
            'learning_rate': [3e-3, 5e-3, 7e-3, 1e-2],     # Higher learning rates
            ...
            'early_stopping_patience': [150, 200, 300],    # Much higher patience
            'scheduler_type': ['onecycle', 'cosine'],       # Advanced schedulers
            'loss_weights': [
        ...
        random.seed(42)
        max_random = min(8, len(all_combinations))
        ...
        print(f"  - Learning rate range: 3e-3 to 1e-2 for faster convergence")
    ...
    def train_with_config(self, model: TunedLNPRegressor, train_data, val_data, 
        ...
        else:  # plateau fallback
            scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                optimizer, mode='min', factor=0.3, patience=20, min_lr=1e-8
            )
        ...
        best_val_r2 = -np.inf
        patience_counter = 0
        max_epochs = 100  # More epochs for breakthrough
        ...
        for epoch in range(max_epochs):
            ...
            for batch_idx in range(n_batches):
                ...
                total_loss.backward()
                torch.nn.utils.clip_grad_norm_(model.model.parameters(), max_norm=2.0)  # Higher clip
                optimizer.step()
